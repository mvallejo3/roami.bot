...
import { createAndSaveApiKey } from "@/app/actions/agents";
...
export default function AgentChat({ agent, apiKey, agentId }: AgentChatProps) {
  ...
  useEffect(() => {
    if (agent && apiKey && chatService === null) {
  ...
  }, [agent, apiKey, chatService]);

  const handleStartChat = async () => {
    startTransition(async () => {
      try {
        await createAndSaveApiKey(agentId, agentUuid);
        // Refresh the page to get the new API key from cookies
        router.refresh();
      } catch (error) {
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              <h2 className="text-2xl font-semibold mb-4 text-accent-primary">
                Begin chatting with your agent.
              </h2>
              <button
                onClick={handleStartChat}
                disabled={isPending}
                className="bg-accent-primary text-foreground-bright px-6 py-3 rounded-lg font-medium hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
